...
export const roomNameMap: Record<string, string> = {
  ...
  '2층 올리브홀(초등부)': '2F Olive Hall (Starlight)',
  '2층 초등부 교사실': '2F Starlight Teachers Room',
  '2층 메자닌': '2F Mezzanine',
  '예배실': 'Sanctuary',
  '친교실': 'Cafeteria',
...
const roomNotices: Record<string, Record<Lang, RoomNotice>> = {
  ...
  '비전홀 영아부실': {
    ko: {
      ...
      footer: '2세 미만의 어린 영아와 부모님들이 함께 예배드리는 공간입니다. 깨끗하게 사용해 주시고, 모임 후에는 모임 전과 같은 상태로 정리해 주시기 바랍니다.',
    },
    en: {
...
const ko = {
  ...
  blackoutUnavailable: '예약 불가',
  blackoutAdminOnly: '관리자만 예약 가능',
  blackoutOverlap: (label: string) => `${label} 시간에는 예약할 수 없습니다. 다른 시간이나 장소를 선택해 주세요.`,
  ...
  adminPastInAllTab: (n: number) => `지난 예약 ${n}건은 '전체' 탭에 있습니다.`,
  ...
  blackoutWeekly: '매주',
  blackoutDaily: '매일',
  blackoutEveryWeekday: (day: string) => `매주 ${day}`,
  blackoutRangeHint: '유효기간은 비워두면 무기한입니다. 부흥회처럼 한동안만 막을 때 채우세요.',
  blackoutAdd: '추가',
  blackoutSaving: '저장 중...',
  blackoutNone: '등록된 예약 불가 시간대가 없습니다.',
  errBlackoutLabel: '이름을 입력해주세요.',
  toastBlackoutAdded: '예약 불가 시간대가 추가되었습니다.',
  toastBlackoutAddedWithExisting: (n: number) =>
    `추가되었습니다. 이 시간대에 이미 예약 ${n}건이 있습니다 — 기존 예약은 유지되고 새 예약만 차단됩니다.`,
  toastBlackoutDeleted: '예약 불가 시간대가 삭제되었습니다.',
...
const en: typeof ko = {
  ...
  blackoutUnavailable: 'Not bookable',
  blackoutAdminOnly: 'Admin only',
  blackoutOverlap: (label: string) => `${label} is not bookable at this time. Please choose another time or room.`,
  ...
  btnEditSubmit: 'Request Change',
  btnEditSubmitting: 'Submitting...',
  ...
  adminPastInAllTab: (n: number) => `${n} past ${n === 1 ? 'reservation is' : 'reservations are'} in the All tab.`,
  ...
  blackoutTitle: 'Unavailable Time Windows',
  blackoutDesc: 'Blocks members from booking times the church already uses regularly, such as worship services. Administrators can still book them.',
  blackoutLabelPlaceholder: 'Name (e.g. Sunday Service)',
  blackoutAllRooms: 'All rooms',
  blackoutWeekly: 'Weekly',
  blackoutDaily: 'Daily',
  blackoutEveryWeekday: (day: string) => `Every ${day}`,
  blackoutRangeHint: 'Leave the dates empty for no end. Fill them in to block a season only, such as a revival week.',
  blackoutAdd: 'Add',
  blackoutSaving: 'Saving...',
  blackoutNone: 'No unavailable windows yet.',
  errBlackoutLabel: 'Please enter a name.',
  toastBlackoutAdded: 'Unavailable window added.',
  toastBlackoutAddedWithExisting: (n: number) =>
    `Added. ${n} reservation${n === 1 ? '' : 's'} already fall inside this window — existing ones are kept and only new bookings are blocked.`,
  toastBlackoutDeleted: 'Unavailable window removed.',
