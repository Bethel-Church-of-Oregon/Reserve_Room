...
- `src/app/api/reservations/[id]/cancel/route.ts` — POST 취소 (**회차 1건만**, 즉시 cancelled 처리). 반복 예약의 회차도 가능하나 시리즈 전체는 불가 (`scope` 파라미터 없음)
...
- 현재 숨김: **비전홀 유아부실** 하나뿐 (2026-09). `ensureDbReady()`에서 `app_settings.rooms_hidden_init_v1` 마커로 **1회만** 실행 — 관리자가 나중에 숨김을 풀어도 재시작 때 다시 숨겨지지 않음
- **비전홀 유치부실은 다시 공개됨** (2026-09). 되돌리는 방법이 두 곳인 이유:
  - `rooms_hidden_init_v1` 목록에서 뺌 → **새 DB** 는 애초에 숨기지 않음
  - `rooms_unhide_preschool_v1` 마커 마이그레이션 추가 → **기존 DB** 를 되돌림. 앞의 블록은 마커가 이미 찍혀 있어 통째로 건너뛰므로 목록만 고쳐서는 반영되지 않음
  - 이 블록은 `SCHEMA_VERSION` 게이트 **안**이라 버전을 올려야 돈다 (`2026-09-08` → `2026-09-22`)
...
- 은퇴(숨김) 장소는 원래 자리를 유지
- **예약 목록 정렬의 2차 기준도 `sort_order`** — `ORDER BY r.start_time, rm.sort_order, r.id` (2026-09 추가)
  - 예전에는 `ORDER BY r.start_time` 뿐이라 **시작 시각이 같은 예약의 순서가 정의돼 있지 않았음.** 일요일 10:00 의 `새가족 교육`·`나의 사랑하는 책`(다른 방)이 **주마다 위아래가 뒤바뀌어** 보였음
  - 범인은 **Postgres 의 `Sort` 가 quicksort 라 stable 하지 않다는 것.** 실행 계획에 `Sort Method: quicksort` 가 찍힘. 입력은 항상 같았음(Bitmap Heap Scan → 물리 순서, 두 시리즈가 각각 다른 블록에 뭉쳐 있음)
  - **뷰는 고칠 수 없음.** `MonthView` 의 비교 함수는 동점에 0 을 반환하고 JS `sort` 는 stable 이라, 서버가 준 순서를 **그대로 보존**함 → 서버가 유일한 수정 지점
  - `r.id` 를 마지막에 두는 이유: **`ROOM_ORDER` 에 없는 장소는 `sort_order = 0`** 이라 그것만으로는 다시 동점이 생김
  - 비용 실측: 공개 1개월 0.130 → 0.135 ms, 관리자 1,700행 1.65 → 2.02 ms. **`Sort` 노드는 원래 있었고 비교 컬럼만 늘어남** — 조인 추가 없음(`rooms` 는 이미 조인), 인덱스 변경 없음, `Sort Method`·메모리(465kB) 동일, 디스크 스필 없음
  - `getFirstOfSeriesFrom...`(`LIMIT 1`)에는 **일부러 안 넣음** — 한 시리즈는 한 장소이고 배타 제약이 같은 시각 중복을 막으므로 동점이 생길 수 없음
  - **SQL 주석을 템플릿 리터럴 안에 쓸 때 백틱 금지** — 문자열이 거기서 끊김
...
- `translateRoomName(name)` 이 **단일 창구.** `roomNameMap`을 직접 조회하지 말 것
  ...
  - `buildingNameMap`(비전홀→Vision Hall, 은혜성전→Grace Chapel) + `roomNameMap` 조합으로 변환
...
## 예약 불가 시간대 (blackout)
- **예배처럼 정기적으로 쓰는 시간을 교인이 예약하지 못하게 막는 규칙.** 관리자는 면제 (2026-09 추가)
- 현재 규칙: **주일예배 · 비전홀 대예배실 · 매주 일요일 08:00~15:30 · 무기한** (`blackout_sunday_service_v1` 마커로 시딩)
- **행 하나가 "매주 영원히"를 표현함.** 예약으로 모델링하지 않은 이유:
  - 반복 예약은 500회 상한이라 몇 년마다 채워 넣어야 함
  - 백업·관리자 검색·취소·변경 경로가 **전부** 그 행들을 걸러내야 함
  - 교인 눈에 예배가 "담당자가 있는 예약"으로 보임
- **DB 제약을 만들지 않았음.** `reservations` 배타 제약은 두 요청이 동시에 SELECT 를 통과하는 **경합** 때문에 필요한데, blackout 은 정적 규칙이라 check 와 write 사이에 바뀔 게 없음 → 서버 검사 한 번으로 충분
- `src/lib/blackout.ts` — **순수 로직 단일 출처.** `db.ts` 가 아니라 여기 있는 이유: `db.ts` 는 Neon 드라이버를 import 하므로 클라이언트 컴포넌트가 함수를 못 가져옴. 예약 폼이 **서버가 강제하는 것과 똑같은 규칙**을 클라이언트에서 돌려야 함
  - 겹침 판정은 `'YYYY-MM-DDTHH:MM:SS'` **전체 문자열 비교** (`HH:MM` 만 비교하지 않음) — 생성 라우트는 시작·종료가 같은 날짜일 것을 요구하지 않으므로 자정을 넘기는 예약에도 맞음
  - 반열림 `[)` — 15:30 에 끝나는 예배는 15:30 시작 예약을 막지 않음
  - `weekdayOfKey()` (`date.ts`) 로 요일 판정 — 성분에서 만들어 서버 시간대와 무관
- **`monthly` 주기는 일부러 없음.** "넷째 주일"/"25일"/"마지막 금요일"로 갈리고, 반복 예약이 쓰는 날짜 기준은 매월 다른 요일에 떨어져 예배 시간엔 쓸모없음 → 대신 `date_from`/`date_to` (부흥회 주간 등)
- **강제 지점**
  ...
  - **관리자 면제는 `asAdmin` (= 서명된 쿠키 AND `body.admin_mode === true`)**, 쿠키만으로는 안 됨 (2026-09 수정). 사무실이 예배 시간에 대예배실을 잡아야 할 때가 있어 면제 자체는 필요함
    - 처음에는 `isAdmin`(쿠키만) 이었고, **관리자 패널에 한 번 로그인한 브라우저는 일반 폼에서도 blackout 이 통째로 풀렸음.** 1달 제한·텔레그램 알림 억제와 **정확히 같은 실수 세 번째**
    - 이 경우가 앞의 둘보다 나빴던 이유: **화면은 빨간 경고를 그대로 띄웠음.** 클라이언트의 `isAdmin` 은 `?admin=true` 까지 요구하므로 경고는 떴고, 서버만 통과시켜 **"예약 불가"라고 적힌 채로 저장**됐음
    - 실측 진리표 — 교인 `409` / 관리자 쿠키+일반 폼 `409` / 관리자 모드 `면제` / `admin_mode` 만 위조 `409`
  - 같은 이유로 `getRooms()` 의 숨김 장소 검증도 `asAdmin` — 일반 폼을 쓰는 관리자는 교인으로 취급
- **기존 예약은 절대 무효화하지 않음.** blackout 은 **새 쓰기에만** 적용
  ...
  - 참고: 일요일 10:00~11:30 의 1,020건(새가족 교육·나의 사랑하는 책)은 **새가족실·2층 교실 4** 라서 이 규칙과 무관. 대예배실 창 안의 기존 예약은 0건
- **공개 읽기 라우트 `GET /api/blackouts`** 에 `dynamic = 'force-dynamic'` **과** `fetchCache = 'force-no-store'` 둘 다 필요 (Neon 이 fetch 로 통신 → 없으면 새 규칙이 한동안 안 보임). 서버 쪽 메모이즈도 하지 않음 — 규칙이 20행 미만이라 매 요청 조회가 무시할 수준이고, 캐시하면 새 규칙이 인스턴스 재활용까지 안 먹음
- 관리자 화면 '설정'의 세 번째 섹션에서 추가·삭제. 수정은 없음 (삭제 후 재추가)
- 예약 폼 현황 패널에 호박색으로 표시. 관리자에게는 `예약 불가` 가 아니라 `관리자만 예약 가능` — 면제 대상에게 불가라고 하면 거짓말
- **2단계 (미구현): 일간·주간 캘린더 배경 띠.** `globals.css` 의 미사용 `.reservation-pending` 빗금을 재활용할 수 있음

## 동시 예약 대비 (중요)
- **이중 예약은 3중으로 막힘.** ①앱 `checkConflict()` SELECT → ②DB 배타 제약 `reservations_no_overlap` → ③`isOverlapViolation(e)` 를 409 로 변환
  - ①과 ② 사이에 SELECT-then-INSERT 창이 있고, **그 창을 닫는 건 ②뿐임.** 상태 집합이 `checkConflict` 와 문자 단위로 같아서 둘이 어긋날 수 없음
  - 실측: 동시 요청 20개 → 정확히 1건만 저장
- **하지만 진짜 흔한 실패는 경합이 아니라 낡은 정보였음.** 예약 폼이 잡힌 시간을 아예 보여주지 않아서, 10분 전에 들어온 예약과도 부딧혔고 다 입력한 뒤에야 409 를 받았음 (2026-09 수정)
  - 충돌 창은 밀리초가 아니라 **폼을 열고 있는 1~3분 전체**
- **① 예약 폼의 예약 현황 패널.** 장소+날짜를 고르면 그 조합의 예약을 불러와 시간 필드 아래에 표시하고, 선택한 시간과 겹치는 항목은 빨간색 + 제출 전 경고
  - **`to` 는 반드시 다음 날.** 범위 필터가 `start_time <= to` 를 문자열로 비교하므로 `'2036-03-30T10:00:00' > '2036-03-30'` → **같은 날짜를 주면 0건**이 돌아옴 (실측 확인)
  - 겹침 판정은 반열림 `[)` — `checkConflict`·DB 제약과 같은 규칙. 11:30 에 끝나는 예약과 11:30 에 시작하는 예약은 겹치지 않음
  - 불러오기 실패는 **빨간 오류로 띄우지 않음.** 정보의 손실이지 안전의 손실이 아니며, 서버가 제출 때 다시 검사함
- **② 409 를 받으면 패널을 재조회** (`takenReload` 증가). 무엇이 그 슬롯을 가져갔는지 보여주지 않으면 다른 시간을 고를 수 없음
- **좌석 선점(hold/lock)은 일부러 안 만듦.** 폼을 열고 나가버린 사람의 hold 가 실제 예약을 막는 새 고장 방식이 생기고, 하루 10건 규모에서 흔한 케이스를 나쁘게 만들어 드문 케이스를 막는 거래가 됨
- 반복 예약 bulk 는 **단일 statement** 라 500건 중 한 건만 늦게 충돌해도 전체 롤백 → "다시 시도해 주세요"
...
- **예약 가능 날짜 창**: 일반 사용자는 **오늘 ~ 1달 이내**. 3중 검증 (2026-09 수정 — 예전에는 위쪽 경계는 두 겹이 **둘 다 통과**했고, **아래쪽 경계는 서버에 아예 없었음**)
  - ① date input 의 `min`/`max` — **혼자서는 아무것도 막지 않음.** 폼이 `noValidate` 라 브라우저 제약 검증이 꺼져 있고, 데스크톱은 범위 밖 날짜를 **직접 타이핑하면 그대로 들어가며**, **iOS 사파리의 휠 피커는 범위 밖 값을 그냥 돌려줌** (실제 신고 경로 — 아이폰에서 지난달 예약이 저장됐음)
    - **네이티브 피커의 회색 처리는 우리가 손댈 수 없음.** `min`/`max` 를 주는 것이 최대치이고 그 UI 는 브라우저 소유 (iOS 는 자체 휠). CSS 로 강제할 방법이 없으므로 **`min`/`max` 는 안내, 검증은 ②③**
  - ② `validate()` 의 `form.date < todayStr()` / `> oneMonthLaterStr()` — ①이 안내일 뿐이므로 화면에서 실제로 막는 건 여기. **제출 버튼을 비활성화하지 않음** — 이유 없이 죽은 버튼은 왜 안 되는지 알려주지 않으므로, 필드에 인라인 오류를 띄우는 쪽이 낫음
  - ③ 서버 `if (!asAdmin)` — 아래·위 경계 **둘 다** 여기서 확정. **`asAdmin = isAdmin && body.admin_mode === true`**
    - 아래쪽은 **날짜 기준이지 "이미 끝났는지" 기준이 아님.** 오늘 오전 9시 예약을 오후에 넣는 것은 허용 — `canEdit`·`canRequestCancel`·시리즈 취소 라우트도 전부 `start_time.slice(0, 10)` 을 태평양 날짜키와 비교함
...
- 충돌 감지: 같은 회의실 + 같은 시간대 이중 예약 방지 — **애플리케이션 + DB 2중 방어**
  ...
  - **시간 범위 인덱스 `reservations_time_range_idx (start_time, end_time)`** (2026-09 추가). 캘린더 4개 뷰가 전부 날짜 범위로 조회하는데 인덱스가 없어서 매번 전체 테이블을 훑었음
  - 실측 (1,835행, 1개월 범위): **Seq Scan 53버퍼·10.94 ms → Bitmap Index Scan 7버퍼·0.53 ms** (20배). 1개월 창이 18행을 얻으려고 1,835행을 읽고 있었음
  - `start_time`이 선두인 이유: 범위 조건과 **`ORDER BY start_time`을 동시에** 처리함. `end_time`을 함께 둔 이유: `end_time >= from` 판정을 인덱스 튜플에서 끝내 후보 행마다 heap 을 찾지 않음
  - **`status` 부분 인덱스로 만들지 않았음.** 부분 인덱스는 쿼리가 술어를 그대로 반복할 때만 쓰이므로, status 목록을 고치는 날 인덱스가 조용히 안 쓰이게 됨
  - 비용: **58 B/행** (행당 총 384 → 442 B). 500행 bulk INSERT 가 인덱스 3개를 유지하게 되지만 무시할 수준
  - `전체` 탭 조회(1,732/1,835행)는 **여전히 Seq Scan 이 맞음** — 테이블의 94%를 읽는 쿼리에 인덱스는 손해
- 기존 겹침 행이 있으면 제약 추가가 실패하므로 `ensureDbReady()`에서 오류를 로그만 남기고 진행 (앱 레벨 검사는 유지)
  - 충돌 메시지는 예약신청 버튼 바로 위에 표시
- **반복 예약: 회차 단위 취소·변경은 교인도 가능, 시리즈 전체는 관리자 전용** (2026-09)
  - 한때 회차 취소까지 403으로 막았다가 되돌렸음. 매주 모임에서 한 주 빠지는 건 일상적인 요청이라 **담당자가 병목**이 되고, 놓기 불편한 예약은 **방이 비어 있는데 잡힌 것으로 표시**되는 손실로 이어짐 — 예약 시스템의 목적 자체를 깎음
  - **시리즈 전체 취소는 여전히 관리자 전용.** `scope` 파라미터를 제거해 공개 경로는 **구조적으로** 회차 하나만 처리하며, 더 요구할 수단이 없음 (`PATCH /api/admin/series/[id]`)
  - 예전에는 예약자 이메일만 알면 공개 폼에서 **시리즈 전체를 한 번에** 취소할 수 있었음. 실제로 새가족 교육 226건이 그 경로로 사라짐 — 그 경로는 없어졌음
  - **남은 위험은 이메일이 유일한 자격증명이고 시리즈 전 회차가 같은 이메일을 쓴다는 것.** 이메일을 아는 사람이 52주를 하나씩 취소하는 건 여전히 가능 → 그래서 **알림이 대책**임
    - 회차 취소이면 담당자 알림에 그 사실을 명시 → 담당자가 **첫 건에서** 알게 됨 (예전에는 다 사라진 뒤에 알았음)
    - 문자 태그 `[취소·반복]` (3자 추가, 1세그먼트 유지), 텔레그램 `🔁 반복 예약 중 이 1회만 취소되었습니다`, 신청자 확인 메일에도 같은 안내
    - **"이 1회만"을 명시하는 이유: 무서운 쪽 해석이 틀린 해석임.** 담당자가 매주 모임 제목을 취소 알림에서 보면 시리즈가 통째로 날아간 줄 알게 됨
  - `CancelRequestModal`은 **누르기 전에** 안내함 (`cancelSeriesOneOnly`) — 매주 모임에서 "취소"는 중의적임
  - 캘린더 4개 뷰의 `canRequestCancel`·`canEdit` 모두 `series_id`를 보지 않음
...
- **목록 뷰**: 일간/주간/월간 옆 "목록" 버튼으로 전환. 오늘 이후 전체 예약을 주 단위 헤더 + 날짜별 카드로 표시. 네비게이션(‹ 오늘 ›) 숨김. fetch 범위: 오늘~1년 후. 각 카드: 제목 / 시간 / 장소 (3줄 구성, 좌측 5px 방 색상 border-l). 카드 클릭 시 선택(배경 진해짐) + 변경하기·취소 신청하기 버튼 표시 (**장소 아래 자기 줄, 우측 정렬** — 월간 모달과 동일. 예전에는 장소와 같은 줄이라 둘 다 쫄렸음, 2026-09 수정). 주 단위 sticky 헤더: `top: -0.2rem`으로 스크롤 시 살짝 올라간 위치에 고정
- 우측 상단: 장소 예약 신청 버튼 + 언어 토글 (**관리자 버튼은 없음** — 아래 참조). 예약 신청 버튼 클릭 시 `RulesModal` 표시 (장소 사용수칙 6개 항목 + 동의 체크박스) → 동의 후 `/reserve`로 이동
  - `rulesItems[].body`는 **문자열 배열** — 항목 여러 개가 서로 다른 두 가지를 말하고 있어서, 한 문단에 이어 붙이면 두 번째가 묻힘 (예: 4번의 전원 차단 / 화기 금지)
  - `rulesOutro`는 영문에만 있음. 한국어 원문에 맺음 문단이 없어서 **빈 문자열로 두고 모달이 건너뜀** — 없는 문장을 지어내지 않기 위함
- 공지 배너: 큰 행사는 사용신청서(Google Drive 링크) 제출 안내
- **관리자 진입은 푸터의 조용한 링크 하나** (2026-09). 헤더의 진한 `관리자 모드` 버튼을 없앰
  - **보안이 아니라 정리 목적임.** `/admin` 은 가장 먼저 찍히는 경로이고 **저장소가 공개**라 소스에 그대로 있음 → 숨김의 가치가 0. 진짜 경계는 `ADMIN_PASSWORD` + 서명 쿠키 + 모든 관리자 라우트의 서버 검사이고 버튼과 무관
  - 헤더에서 뺀 이유: 방문자 거의 전원이 못 쓰는 컨트롤이 **`장소 예약 신청`과 같은 줄에서 폭을 다툼** (헤더는 320px 한 줄에 맞춰져 있음)
  - **완전히 지우지는 않았음.** 버튼은 스스로를 설명하므로, 지우면 담당자가 바뀔 때 그 지식이 누군가의 북마크에만 남음
  - **문구는 `관리자 로그인`(`linkAdminLogin`), `관리자 모드`가 아님.** 푸터는 상태·메타 정보가 사는 자리라 명사구를 두면 "지금 내가 관리자 모드인가?"로 읽힘 — 교인은 예약을 망설이고 담당자는 이미 로그인된 줄 앎. 서술어가 붙어야 목적지로 읽힘 (`btnAdmin`·`btnAdminShort` 는 이제 아무도 안 씀)
  - **뷰포트 고정 푸터가 아니라 각 뷰 스크롤 컨테이너의 마지막 자식.** 캘린더 콘텐츠 끝에 붙어 함께 스크롤됨
    - 줄을 차지하는 푸터로 두면 **캘린더 높이를 상시로 42px 가져감** (모바일 월간 셀이 100px)
    - 스크롤러가 일간·주간·월간에 각각 따로 있어서 `footer?: React.ReactNode` **prop 으로 내려보냄.** 정의는 `page.tsx` 의 `AdminFooterLink` 한 곳
    - 일간·주간·월간은 스와이프 `translateX` **밖**에 둠 — 날짜를 넘길 때 푸터가 같이 미끄러지면 안 됨
    - **목록 뷰에는 넣지 않음** (일간·주간·월간만)
  - 높이 27px: 패딩을 바가 아니라 **버튼에** 몰아서 바는 짧게, 탭 영역은 26px 유지. `leading-none` 필수 — **임의값 `text-[10px]` 은 `font-size` 만 설정**하므로 없으면 preflight 의 `line-height: 1.5` 를 물려받아 계산이 어긋남
...
- **월간 뷰 스크롤러는 하나뿐임 — `page.tsx` 의 래퍼에 `overflow-y-auto` 를 다시 붙이지 말 것** (2026-09 수정)
  - 예전에는 `page.tsx` 래퍼와 `MonthView` 안쪽에 **스크롤러가 둘** 겹쳐 있었음. `MonthView` 가 정확히 `h-full` 이라 바깥이 넘칠 일이 없어 증상이 없었을 뿐 → 푸터를 넣자마자 **스크롤바 두 개**가 나옴
  - `MonthView` 안에서 **스크롤러와 그리드를 분리함.** 예전에는 한 요소가 스크롤러 + 그리드 + 스와이프 대상 3역을 겸해서 주(week) 뒤에 아무것도 넣을 자리가 없었음
    - 그리드에 **`min-h-full` 필수.** 플렉스 아이템이 아니게 되면 높이가 콘텐츠 기준이 되어 `1fr` 행이 전부 `--month-cell-min-h` 로 주그러듦
    - **`month-grid` 클래스는 그리드 쪽에 남길 것** — 그 클래스가 `--month-cell-min-h` 를 *정의*하고, 행 템플릿과 주 행들이 그 변수를 읽음
...
- **장소별 추가 주의사항 + 확인 체크박스** (2026-09). `비전홀 유치부실`·`비전홀 영아부실` 두 곳만
  - 문구는 `i18n.ts` 의 `roomNotices` — **저장된 이름(건물 접두사 포함)을 키로** 씀. `roomNameMap` 이 접두사 없는 키라서 한 번도 매칭 안 됐던 것과 같은 함정
  - **한/영을 방 하나 밑에 나란히** 둠 (`Record<string, Record<Lang, RoomNotice>>`) → 방을 추가하면서 번역을 빼먹으면 **컴파일 오류**. ko/en 을 200줄 떨어뜨려 두면 안 잡힘
  - 위치는 **장소 박스와 날짜 박스 사이.** 방금 고른 장소의 결과이므로 그 자리가 맞고, 제출 버튼 옆에 두면 폼을 다 채운 뒤에야 조건을 알게 됨
  - **장소를 바꾸면 체크가 풀림** (`useEffect` on `form.room_id`). 안 그러면 영아부실 확인 후 유치부실로 바꿨을 때 *교사실 물품 금지* 항목을 못 보고 통과함
  - **서버 강제는 하지 않음** — 사용수칙 모달 동의와 같은 성격(honor system)이고, 기록하려면 컬럼이 필요한데 얻는 게 없음
  - 관리자도 동일하게 체크해야 함. 방을 아끼는 문제이지 권한 문제가 아니라 면제를 두지 않음
  - **주의사항이 없는 방은 아무것도 렌더되지 않음** — 모든 방에 띄우면 아무도 안 읽음
- **시간 선택 규칙은 `src/lib/timeOptions.ts` 단일 출처** (2026-09). 예약 폼과 변경 모달에 `generateTimeOptions` 가 **바이트 단위로 복제**돼 있던 걸 합침
  - **종료 목록은 시작 이후만** (`endTimeOptions(start)`). 예전에는 양쪽 다 96개 전체를 보여줘서, 08:00 시작에 07:00 종료를 고를 수 있었고 `validate()` 가 뒤늦게 거절했음
  - **시작 목록에서 `23:45` 제외** (`START_TIME_OPTIONS`). 폼이 시작·끝을 **같은 날짜**로 조립하므로 23:45 시작은 그날 고를 끝이 없음 → 종료 목록이 통째로 빔. 원래도 불가능했던 조합을 목록에서 드러낸 것
  - **시작이 끝을 넘어설 때만 끝이 따라감** — 조건은 `prev.end > next` 이면 그대로 둠. **끝은 사용자가 정한 값이라 함부로 옮기지 않음**
    - 따라갈 때는 **원래 길이를 유지** (`endTimeForNewStart`). `09:00~10:00` → 시작 14:00 이면 `14:00~15:00`, **`09:00~09:30` 이면 `14:00~14:30`** — "항상 1시간"이 아님. 기본값이 1시간(`09:00~10:00`)이라 그렇게 보일 뿐
    - 분이 아니라 **그리드 스텝 수로 계산** → 결과가 15분 격자 위에 있음이 보장됨
    - 하루 끝에서 클램프: `09:00~11:00` 에서 시작 23:00 이면 `23:00~23:45`
  - **끝이 목록에 없는 값을 들고 있으면 select 가 빈칸으로 렌더됨** — 위의 따라가기가 없으면 바로 그 상태가 됨
  - `validate()` 의 `errEndBeforeStart` 는 남겨둠 — 화면으로는 도달 불가지만 서버와 함께 두는 안전망
...
- **`input[type="date"]` 는 `appearance-none bg-white min-w-0` 이 함께 있어야 함** (2026-09 수정)
  - 네이티브로 두면 **iOS 사파리가 날짜 입력의 너비를 자기 고유 폭으로 정하고, `w-full` 이 그보다 작게 줄이지 못함** → 박스가 폼 padding 을 넘어 삐져나오고, 회색 배경 + 가운데 정렬 텍스트로 나옴 (옆의 `장소` 셀렉트와 눈에 띄게 어긋남)
  - **데스크톱에서는 고유 폭이 충분히 작아 `w-full` 이 이기므로 안 보임** — 폰에서만 드러나는 부류
  - `min-w-0` 은 그 고유 폭이 기대는 UA `min-width` 를 지움
  - `장소` 셀렉트에는 원래 `appearance-none bg-white` 가 있었고 날짜 입력에만 빠져 있었음. 날짜 입력을 새로 추가할 때 같이 넣을 것
- **`<select>` 도 같은 규칙 — `appearance-none bg-white` + 커스텀 chevron** (2026-09 통일)
  - `appearance-none` 은 네이티브 화살표까지 같이 없애므로 **`relative` 래퍼 안에 `SelectChevron` 을 넣어야 함** (`reserve/page.tsx`). `pointer-events-none` 이라 클릭은 셀렉트로 통과함
  - 패딩은 `pl-3 pr-8` (`px-3 pr-8` 이 아님) — Tailwind 유틸 출력 순서에 의존하지 않게. `pr-8` 은 값이 chevron 아래로 들어가지 않게 하는 자리
  - 예전에는 `장소` 만 이 처리가 돼 있어서, 아이폰에서 **시간 셀렉트 두 개만 회색 배경 + 네이티브 스테퍼**로 나와 어긋났음
  - `장소` 셀렉트는 선택 시 chevron 대신 **장소 색 점**을 보여주므로 `SelectChevron` 을 쓰지 않고 자체 래퍼를 유지함
...
- **관리자 상단 고정**: 헤더 + 버튼 행 + 장소필터/검색 행을 **`sticky top-0 z-30` 래퍼 하나**로 묶음. 그 아래 목록만 스크롤 (2026-09)
  - `전체` 탭이 1,700행대라 탭 전환·검색을 하려면 맨 위까지 되돌아가야 했음
  - **`h-screen overflow-hidden` + 내부 스크롤을 쓰지 않은 이유 두 가지**
    - 조상에 `overflow-hidden`이 생기면 **장소 필터 드롭다운이 잘림** (`absolute` 오버레이라 컨테이너를 벗어나야 함)
    - `100vh`는 iOS Safari 에서 주소창을 포함해 **마지막 행에 손이 닿지 않음**
  - 줄마다 `top-[50px]` 식 오프셋을 주지 않고 하나로 묶은 이유: 버튼이 `clamp()` 폰트라 높이가 폭에 따라 변하고, 좁은 화면에서 줄바꿈되고, 장소 chip 이 한 줄 더 붙음 → 하드코딩한 오프셋이 어긋남
  - **sticky 블록 안에서는 `mb-*` 대신 `py-*`.** 마지막 자식의 bottom margin 은 sticky 박스 밖으로 collapse 돼서 목록이 비쳐 보이는 투명 띠가 생김
  - **데스크톱 표 `<thead>` 도 고정** (`sticky top-0`). 1,700행에서는 컬럼 레이블을 잃는 게 툴바를 잃는 것보다 불편함
    - `overflow-x-auto` 는 **이미 양축 스크롤 박스**임 (한 축이 `visible` 이 아니면 다른 축의 `visible` 이 `auto` 로 계산됨) → 높이만 주면 `thead` 가 이 박스 기준으로 붙음
    - 높이는 `calc(100vh - 툴바높이 - 4.5rem)`, 툴바 높이는 **`ResizeObserver` 로 실측** (위의 이유로 상수가 될 수 없음)
    - **`admin:` 브레이크포인트에서만** 적용. 카드 레이아웃(모바일)은 문서 스크롤 유지 → `100vh` 를 폰에서 안 씀
    - `border-b` 가 아니라 **`shadow-[inset_0_-1px_0_...]`**. Tailwind preflight 가 `border-collapse: collapse` 를 켜는데, collapse 된 border 는 sticky `thead` 에서 스크롤 중에 사라짐 (Chrome)
- **관리자 탭별 기간**: `예약 목록`·`취소 목록`은 **오늘(태평양) 이후만**, `전체`는 **1개월 전부터**
  - 앞의 두 탭은 작업 목록임 — 앞으로 있을 것과 거기서 빠진 것. 지난 날짜는 둘 다 아니므로 `전체`(보관소)로 넘김
  - **날짜 기준이지 "끝났는지" 기준이 아님.** 한 시간 전에 끝난 예약이야말로 오늘 문의가 들어올 건이고, 앱의 다른 "지난" 판정(`canEdit`·`canRequestCancel`·시리즈 취소 라우트)도 전부 `start_time.slice(0, 10)`을 태평양 날짜키와 비교함
  - **fetch 범위는 `ADMIN_HISTORY_MONTHS = 1`** → `?from=<1개월 전>`. 앞의 두 탭은 어차피 오늘 이후만 보므로 이 값이 정하는 건 **`전체` 탭이 과거를 얼마나 담는지** 하나뿐
    - `addMonthsToKey(key, months)` (`src/lib/date.ts`) — **말일을 클램프.** `2026-03-31` 에서 1개월을 빼면 `2026-02-28`. `setMonth`는 3월로 되감아서 하한이 앞으로 튀고 조용히 며칠을 잘라먹음
    - **속도 대책이 아니라 증가 상한임.** 응답은 *미래* 반복 회차가 지배함 (500주 시리즈 = 500행) → 실측 1,833행 중 미래가 1,717행이고, 1개월 제한은 1046 KB → 990 KB (5%)뿐
    - 진짜 무거운 쪽을 줄이려면 `전체` 탭 페이지네이션/가상스크롤이 필요함. 현재 실사용 경로는 **검색으로 좁히기**
    - 1개월보다 오래된 예약은 **관리자 UI 에서만** 안 보임. DB·월간 백업 CSV 에는 그대로 있음
  - 걸러진 게 있으면 빈 목록에 `지난 예약 N건은 '전체' 탭에 있습니다.` 표시. "없습니다"만 보면 데이터가 사라진 것으로 읽힘
  - **시리즈 묶음 행의 건수도 미래 회차만 셈** → `시리즈 취소`가 오늘 이후만 취소하는 동작과 숫자가 일치함 (예전에는 `246건`이라 적힌 행을 취소하면 지난 회차는 남아서 어긋났음)
- **관리자 검색**: 장소 필터와 같은 줄, 오른쪽 남는 공간 전체 (`flex-1 min-w-0`). 세 탭(예약 목록·취소 목록·전체) 모두에 적용되고 **탭을 바꿔도 검색어가 유지됨** — 같은 사람의 예약과 취소를 잇달아 찾는 경우가 대부분
  - 대상은 `searchHaystack()` 단일 출처: 제목·담당자·**이메일**·장소(한/영 양쪽)·노트·취소사유·날짜·시각
    ...
    - 장소는 저장명(`은혜성전 친교실`)과 표시명(`Grace Chapel Cafeteria`)을 둘 다 넣음 — 화면에 보이는 대로 검색해도 걸려야 함
...
- **관리자 '설정' 화면은 `admin`(1000px)부터 2단** (2026-09). 예전에는 브레이크포인트가 **하나도 없어서** 폰 레이아웃이 1280px 화면에도 그대로 나왔음 — 512px 폼이 왼쪽에 붙고 오른쪽 760px 이 빔
  - 왼쪽 열: 예약 코드 + 알림 수신자 / 오른쪽 열: 예약 불가 시간대 (가장 큼)
  - **새 브레이크포인트를 만들지 않고 `admin` 을 재사용함** — 예약 목록의 표↔카드 전환과 같은 선이라 패널 안에서 "데스크톱"의 정의가 하나로 유지됨
  - **각 섹션의 `max-w-lg` 는 유지.** 열은 섹션을 *배치*하려는 것이지 *넓히려는* 게 아님 — 입력칸을 열 전체로 늘리면 오히려 쓰기 나빠짐
  - `mx-auto admin:mx-0` — 1000px 미만(특히 640~999px)에서는 가운데 정렬, 그리드에서는 해제. 그리드 안에서 `mx-auto` 가 남으면 **두 열이 서로 바깥으로 벌어짐**
  - 예약 불가 섹션의 `mt-10 pt-8 border-t` 는 `admin:` 에서 꺼야 함 — 오른쪽 열 맨 위가 되면 두 열의 시작 높이가 달라지고 한쪽에만 가로줄이 그어짐
  - 상단 sticky 버튼 행은 이 배치와 무관 (건드리지 않음)
- 제목 말줄임: 모든 뷰(일간 캘린더 블록, 주간, 월간, 목록 보기, 팝오버, 관리자 테이블/카드)에서 긴 제목은 `truncate`로 처리
- **페이지 헤더의 제목·부제는 `min-w-0` + `truncate` 쌍으로.** 예약 페이지 헤더에 이게 빠져 있어서 긴 교회명이 320px 에서 두세 줄로 접혔음 (2026-09 수정). `min-w-0` 이 없으면 flex 자식이 `min-width: auto` 라 콘텐츠보다 작아지기를 거부하고 `truncate` 가 무력화됨 — 관리자 헤더에는 원래 있었음
  - **문구 길이로 레이아웃을 맞추지 말 것.** 짧게 줄이면 그때는 되지만 다음에 긴 문구를 넣으면 다시 접힘
- **영문 교회명은 `Bethel Room Reservation` 한 가지로 통일** (2026-09). 예전에는 `Bethel Room Reservation` / `Oregon KBPC ...` / `Oregon Bethel Presbyterian Church ...` 세 가지가 화면마다 달랐음. 한국어는 화면 역할에 맞는 자연스러운 표현을 유지 (`오레곤벧엘장로교회 예약 관리` 등)
