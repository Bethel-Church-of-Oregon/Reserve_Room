...
- **반복 예약의 변경·취소는 모두 관리자 전용** (2026-09). 공개 경로는 `reservation.series_id`가 있으면 **403**
  ...
  - 변경 차단은 `applyReservationEdit()`에서 `requesterEmail !== null`(공개 경로)일 때만 — 이메일 확인이 있는 그 자리와 같은 조건
  - 캘린더 4개 뷰의 `canRequestCancel`·`canEdit`에 `!series_id` 추가 + 팝오버에 "반복 예약은 담당자에게 문의해 주세요" 안내 노출 (버튼이 그냥 사라지면 혼란스러움)
...
- **묶기는 예약 목록(`approved`) 탭에서만.** 취소 목록과 전체 탭은 회차별로 그대로
  ...
  - **전체 탭: 관리자가 시리즈의 개별 회차를 수정·삭제하는 유일한 창구.** 교인이 반복 예약을 못 건드리게 된 뒤로 이 경로가 필요해짐
