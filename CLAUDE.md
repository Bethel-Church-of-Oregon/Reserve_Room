...
- **관리자 탭별 기간**: `예약 목록`·`취소 목록`은 **오늘(태평양) 이후만**, `전체`는 **전 기간**
  ...
  - **fetch 는 기간 제한 없음** (`/api/admin/reservations`, `from` 없이). 예전엔 `?from=<어제>`였는데, 그러면 보관소여야 할 `전체` 탭에서도 과거가 안 보임
    - 제한을 걷어낸 비용은 9% (약 1,300행 기준 696 KB → 758 KB). 미래 반복 시리즈가 응답 대부분을 차지해서 과거를 잘라도 거의 안 줄었음
    - 대신 `전체`는 무제한으로 커짐 → 검색으로 좁히는 게 실사용 경로
