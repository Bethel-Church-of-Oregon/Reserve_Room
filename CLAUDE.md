...
- **장소 필터**: "장소 필터 ▾" 버튼 클릭 시 패널 펼침, 장소 chip 클릭으로 멀티 필터링, "전체 보기"로 초기화. 같은 줄 오른쪽에 확정/승인대기 범례 + 불러오는 중 표시. 패널 접힌 상태에서도 선택된 장소 chip은 그대로 표시 (클릭 시 해제 가능). 모바일에서는 패널이 오버레이 드롭다운으로 표시 (캘린더 위에 absolute 포지션, backdrop 클릭 시 닫힘). 패널 내 chip·전체보기 클릭은 `stopPropagation`으로 backdrop 전파 차단. header `z-50`으로 backdrop(`z-40`) 위에 위치
